...
export namespace TransfersHistoryList {
...
@connect(
  ...
  (dispatch: any, ownProps: any): any => ({
    ...
    getReport: (
      ...
      let userId = (that.props.match.params as any).id;
      ...
      let params: any = {
        ...
        state: that.props.appliedColumnFilter.state && that.props.appliedColumnFilter.state.length > 0 ? that.props.appliedColumnFilter.state : [-1],
        transactionType: that.props.appliedColumnFilter.typeId && that.props.appliedColumnFilter.typeId.length > 0 ? that.props.appliedColumnFilter.typeId.join(',') : ''
      };
      let data = {
        url: concatParamsToURL(api.GET_HISTORY_TRANSFERS, params),
        method: ajaxRequestTypes.METHODS.GET
      };
    ...
    getChangedReport: (that: any, val: ITable) => {
      ...
      let params = {
        ...
        transactionType: val.filter.typeId !== null && val.filter.typeId.length > 0 ? val.filter.typeId.join(',') : ''
      };
      let data = {
        url: concatParamsToURL(api.GET_HISTORY_TRANSFERS, params),
        method: ajaxRequestTypes.METHODS.GET
...
class TransfersHistoryListVO extends React.Component<TransfersHistoryList.Props, TransfersHistoryList.State> {
  private debouncedSearchPlayer: DebouncedFunctionWithMethods<(val: string) => void>;

  constructor(props: TransfersHistoryList.Props, context?: any) {
    super(props, context);
    ...
    this.state = {
      ...
      playersSelectOptions: this.props.selectedPlayers
        ? normalizeSelectedPlayers(this.props.selectedPlayers, !isAdmin)
        : [],
    };

    this.debouncedSearchPlayer = debounce(this.props.onSearchPlayers(this, !isAdmin), 1000);
  }
  ...
  getGameServerNameById(id: number): any { // TODO: does not used anywere
    for (let i = 0; i < ALL_GAMES.length; i++) {
      let item = ALL_GAMES[i];
      if (parseInt(item.id) === id) {
  ...
  onChangeSelectPlayers = (val: any, options: any) => {
    this.props.selectPlayers(
      options.map((option: any) => ({ id: option.props.value, login: option.props.title }))
    )
  };
  ...
  onBlurSelectPlayers = () => {
    ...
    this.setState({ playersSelectOptions: normalizeSelectedPlayers(this.props.selectedPlayers, !isAdmin) });
  };
  ...
  renderTable() {
    ...
    const columns: any = [
      {
        title: 'ID' || (localization.BET_ID_COLUMN as any)[lang],
        dataIndex: 'id',
        sorter: false,
          ...
          let text = (searchText||searchText==='') && value ?
            ...
            {value == searchText ?
              <span id={"search-value"} key={i}
                    style={{backgroundColor: '#fff20c'}}>{`${value}`}</span> : `${value}`
            }
          </span>
            : value;
      ...
      },
      {
        ...
        render(value: any, row: any, i: number) {
          const curValue = row &&
            row['jsonResult'] &&
            row['jsonResult']['external_transaction_id'] ?
              row['jsonResult']['external_transaction_id'] as string :
              '-';
          return {
            children: curValue,
            props: {
      ...
      {
        title: 'Transaction Type' || (localization.TYPE_COLUMN as any)[lang],
        dataIndex: 'typeId',
        sorter: false,
        filteredValue: appliedColumnFilter.typeId || null,
        filters: [
          {text: localization.DEPOSIT_TYPE[lang], value: 1},
          {text: localization.WITHDRAW_TYPE[lang], value: 2},
        ],
        render: (value: any, row: any, i: number) => {
          let spanStyle: any = {
            padding: '5px',
      ...
      {
        title: 'Amount' || (localization.PLAYERS_PROFIT_COLUMN as any)[lang],
        sorter: false,
        dataIndex: 'profit',
        render(value: any, row: any, i: number) {
          let obj = {
  ...
  renderFilter() {
    ...
    const playersSelect = {
      ...
      data: isAdmin
        ? this.state.playersSelectOptions
        : this.state.playersSelectOptions.map(({ id, title }) => ({ id, name: removeLoginSuffix(title), title })),
    ...
    return (
      <Filter
        additionalFields={[idSearchField, timePicker, playersSelect]}
  ...
  render() {
    ...
    return (
      <div className={'gutter-box-padding'}>
        {validUserId ? null : <Row type="flex" justify="space-around" align="middle">
          <TabsComponent
            withBudge={false}
...
export const TransfersHistoryList = withRouter(TransfersHistoryListVO);
