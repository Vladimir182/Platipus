...
import {Row, Breadcrumb, message, Select, Popconfirm, Popover, Form, Input, Icon, Switch, Tag} from 'antd';
  ...
  (dispatch: any): any => ({
    getReport: (that: any, needResetPagination?: boolean, levelObj?: any) => {
      ...
      let params: any = {
        ...
        platform: (levelObj && levelObj.levelKey === StructureLevel.Distributor) ? levelObj.levelValue : that.props.match.params.platform || "all",
      ...
      let data = {
        url: concatParamsToURL(api.GET_STRUCTURE, params),
...
class StructureVO extends React.Component<Structure.Props, Structure.State> {

  constructor(props: Structure.Props, context?: any) {
    super(props, context);
  ...
  goToLevelByRole(){
    ...
    switch(role){
      case roles.ADMIN.name:{
        ...
        level = StructureLevel.Distributor;
        break;
      }
      case roles.DISTRIBUTOR.name:{
  ...
  goToLevel(level: number, id: any, customParamsArr?: any[]) {
    ...
    let url = `${r.route}${r.children.STRUCTURE}`;
  ...
  getLevelKey(): number {
    ...
    let levelKey = 0;
    if (!platform && !distributorId && !officeId && !agentId && !roomId) {
      levelKey = StructureLevel.Platform;
    } else if (platform && !distributorId && !officeId && !agentId && !roomId) {
      levelKey = StructureLevel.Distributor;
    } else if (platform && distributorId && !officeId && !agentId && !roomId) {
      levelKey = StructureLevel.Office;
    } else if (platform && distributorId && officeId && !agentId && !roomId) {
      levelKey = StructureLevel.Agent;
    } else if (platform && distributorId && officeId && agentId && !roomId) {
      levelKey = StructureLevel.Room;
    } else if (platform && distributorId && officeId && agentId && roomId) {
      levelKey = StructureLevel.Cashier;
    }
    return levelKey;
  }
  ...
  renderTable() {
    //table
    //function get ip address si string and return unparced array
    // let ipArrTest = '2index.tsx:1584 174.213.160.140,2001:8003:a894:e901:5560:f71a:2bd1:82ad,49.185.150.130';
    let ipColumnCell = (ipArrStr: string) => {
      if (!ipArrStr) { return ''}
      const ipArr = !ipArrStr.length && [''] || ipArrStr.split(',');
      if (ipArr.length === 1) {
        return (
          <span className="ip-column-cell__main-ip">
            {ipArr[0]}<span style={{opacity: 0}}>..</span>
          </span>
        )
      } else {
        return (
            ...
            <Popover content={(
              <div>
                <ul style={{listStyle: 'none', padding: 0}}>
                  {ipArr.map((reptile) => <li key={reptile.toString()}>{reptile}</li>)}
                </ul>
              </div>
            )}>
              <span style = {{cursor: `${(!(ipArr.length-1) && 'auto') || 'pointer'}`}}>
                {ipArr[0]}...
              </span>
            </Popover>
    ...
    switch (this.getLevelKey()) {
      ...
      case StructureLevel.Office:
        columns = [
          ...
          {
            title: localization.DISTRIBUTOR_COLUMN[lang],
            dataIndex: 'ownerId',
            key: 'ownerId',
            sorter: false,
            render(value: any, row: any, i: number) {
              return {
                children: `${row.ownerName} (#${value})`,
                props: {
                  style: {}
                }
              };
      ...
      case StructureLevel.Platform: {
        columns = [
          ...
          {
            title: localization.DISTRIBUTOR_COUNT_COLUMN[lang],
            dataIndex: 'items',
            key: 'items',
            sorter: false,
            render: (value: any, row: any, i: number) => {
      ...
      case StructureLevel.Room:
        columns = [
          ...
          {
            title: localization.AGENT_COLUMN[lang],
            dataIndex: 'ownerId',
            key: 'ownerId',
            render: (value: any, row: any, i: number) => {
              return {
          ...
          {
            title: localization.CASHIER_COUNT_COLUMN[lang],
            dataIndex: 'items',
            key: 'items',
            sorter: false,
            render: (value: any, row: any, i: number) => {
      ...
      case StructureLevel.Cashier:
        columns = [
          ...
          /* {
            title: localization.BALANCE_COLUMN[lang],
            dataIndex: 'balance',
            key: 'balance',
            editable: true,
            render: (value: any, row: any, i: number) => {
          ...
          /* {
            ...
            render: (value: any, row: any, i: number) => {
              return {
                children: this.renderStatusColumn(value, row.id),
                props: {}
              };
            }
          }, */
          /* {
            title: localization.LAST_VISIT_COLUMN[lang],
            dataIndex: 'lastVisit',
            key: 'lastVisit',
            render: (value: any, row: any, i: number) => {
              return {
          ...
          {
            title: localization.LAST_IP_COLUMN[lang],
            dataIndex: 'lastIp',
            key: 'lastIp',
            render: (value: any, row: any, i: number) => {
              return {
          ...
          }, */
          {
            ...
            sorter: false,
            className: 'ip-column-cell',
            dataIndex: 'ipAddress',
            key: 'ipAddress',
            render(value: any, row: any, i: number) {
              return {
                children: ipColumnCell(value),
                props: {}
              };
            }
          },
  ...
  renderBreadCrumb = () => {
    ...
    if (this.getLevelKey() >= StructureLevel.Distributor) {
      breadcrumb.push({
        levelKey: StructureLevel.Distributor,
        isLink: this.getLevelKey() !== StructureLevel.Distributor,
  ...
  getLevelTitle = () => {
    ...
    switch (this.getLevelKey()) {
      ...
      case StructureLevel.Distributor: {
        title = localization.DISTRIBUTORS_HEADER[lang];
        break;
      }
      case StructureLevel.Office: {
      ...
      case StructureLevel.Agent: {
        title = localization.AGENTS_HEADER[lang];
        break;
      }
      case StructureLevel.Room: {
      ...
      case StructureLevel.Cashier: {
        title = localization.CASHIERS_HEADER[lang];
        break;
      }
    }
    return title;
    ...
    form.validateFields((error: any, row: any) => {
      ...
      switch (this.getLevelKey()) {
        case StructureLevel.Platform:
          break;
        case StructureLevel.Distributor:
          validatedFields = this.state.dataToSave.logins.length !== 0;
          break;
        case StructureLevel.Office:
        ...
        inputs = [
          {
            ...
            placeholder: localization.LOGIN_INPUT[lang],
            label: localization.INPUT_LOGIN[lang],
            onChange: (e: any) => {
          ...
          /* ,{
            ...
            placeholder: localization.BALANCE_INPUT[lang],
            label: localization.INPUT_BALANCE[lang],
            onChange: (e: any) => {
              this.inputValue(e.target.value, 'balance')
            }
          } */];
        break;
      case StructureLevel.Office:
  ...
  renderModalTitle = () => {
    ...
    switch (this.getLevelKey()) {
      case StructureLevel.Platform:
        break;
      case StructureLevel.Distributor:
        title = localization.ADD_DISTRIBUTOR[lang];
        break;
      case StructureLevel.Office:
        title = localization.ADD_OFFICE[lang];
        break;
      case StructureLevel.Agent:
        title = localization.ADD_AGENT[lang];
        break;
      case StructureLevel.Room:
        title = localization.ADD_ROOM[lang];
        break;
      case StructureLevel.Cashier:
        title = localization.ADD_CASHIER[lang];
        break;
    }
    return title;
  };
    ...
    return (
      <div className={'gutter-box-padding'}>
        ...
        {/* <ModalComponent
          elementsInRow={1}
          handleOk={() => {
            this.props.changeIpAddress(this);
          }}
          handleCancel={this.onCancel}
...
export const Structure = withRouter(StructureVO);
