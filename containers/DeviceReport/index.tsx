...
@connect((state): any => ({
  ...
  (dispatch: any): any => ({
    ...
    getReport: (that: any, needResetPagination?: boolean, isAllRecords?: boolean, sortObj?: string[]) => {
      ...
      let data = {
        url: concatParamsToURL(api.GET_DEVICE_REPORT, params),
        method: ajaxRequestTypes.METHODS.GET
      };
...
class DeviceReportVO extends React.Component<DeviceReport.Props, DeviceReport.State> {

  constructor(props: DeviceReport.Props, context?: any) {
    super(props, context);
  ...
  onSaveExcelButton = () => {
    ...
    let filename: string;
    fetch(concatParamsToURL(api.GET_DEVICE_REPORT_FILE, params), request)
      .then((response: any) => {
  ...
  renderTable() {
    ...
    const columns: any = [
      ...
      },
      {
        ...
        render: (value: any, row: any, i: number) => {
          return {
            children: value, // this.renderNameCell(value),
            props: {
              style: {
                fontWeight: '700',
              }
...
export const DeviceReport = withRouter(DeviceReportVO);
