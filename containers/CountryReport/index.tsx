...
import {Row, TreeSelect, Breadcrumb, Col, Tooltip} from 'antd';
...
import {
  ...
  formatCountryData, getCorrectSortKey, /* getOneDayRangeByDate, */ getParameterByName, insertParam,
  isDeviceMobileAndTablet,
  removeParams, reversedSortTableOrder, /*shortFormatNumber,*/
  sortTableOrder,
  generateValueWithCurrency, generateValueWithoutCurrency, generateValueWithPercent, convertSelectedCountryValuesToString
} from 'app/utils'
import {
...
export namespace CountryReport {
  export interface Props extends RouteComponentProps<void> {
    ...
    getReport: (that: any, needResetPagination?: boolean, sortObj?: string[], levelObj?: any) => void;
    ...
    getCountrysList: (that: any) => void;
    selectCountrys:(val: string[]) => void;
...
@connect((state): any => ({
    ...
    countrysList: state.filter.countrysList,
    selectedGames: state.filter.selectedGames,
    selectedCountrys: state.filter.selectedCountrys,
  ...
  (dispatch: any): any => ({
    ...
    getReport: (that: any, needResetPagination?: boolean, isAllRecords?: boolean, levelObj?: any) => {
      ...
      let params: any = {
        ...
        countryIds: that.props.selectedCountrys.length === 0 ? '' : convertSelectedCountryValuesToString(that.props.selectedCountrys, that.props.countrysList),
      ...
      let data = {
        url: concatParamsToURL(api.GET_COUNTRY_REPORT, params),
        method: ajaxRequestTypes.METHODS.GET
      };
    ...
    getCountrysList: (that: any) => {
      let types = ajaxRequestTypes.standartAsyncTypes(ajaxRequestTypes.COUNTRY_LIST);
      let data = {
        url: concatParamsToURL(api.GET_COUNTRY_LIST, {}),
        method: ajaxRequestTypes.METHODS.GET
      };
      req(types, data, that.props.history, dispatch);
    },
...
class CountryReportVO extends React.Component<CountryReport.Props, CountryReport.State> {

  constructor(props: CountryReport.Props, context?: any) {
    super(props, context);
    ...
    goToLevel(level: number, value: any, countryId: number) {
    let roleId = localStorage.getItem('roleId') as string;
    let role: string = (getRoleById(parseInt(roleId)) as string).toUpperCase();
    let r = roles[role];
    if (level === 1) {
      this.props.history.push(`${r.route}${r.children.COUNTRY_REPORT}`);
      this.props.getReport(this, true, ["name", "asc"], {
        levelKey: level,
        levelValue: localization.COUNTRY_COLUMN[lang],
      });
    }
    if (level === 2) {
      this.props.history.push(`${r.route}${r.children.COUNTRY_REPORT}/${countryId}`);
      this.props.getReport(this, true, ["name", "asc"], {
        levelKey: level,
        levelValue: countryId,
      });
    }
  }
  ...
  getLevelValue(): string {
   ...
   getLevelTitle = () => {
    ...
    switch(this.getLevelKey()){
      case 1:{
        title = localization.COUNTRY_COLUMN[lang];
        break;
      }
      case 2:{
  ...
  renderBreadCrumb = () => {
    ...
    if (this.getLevelKey() === 2) {
      breadcrumb.push({
        ...
        levelValue: this.props.metadata.countryName
      })
    }
    return (
      <Breadcrumb>
          ...
          breadcrumb.map((val: any) => {
            return (
              <Breadcrumb.Item key={val.levelKey}>
                {val.isLink ?
                  <a title={val.title}
                     onClick={() => this.goToLevel(val.levelKey, val.levelValue, countryId)}>{val.levelValue}</a>
  ...
  defaultPageSettings = () => {
    ...
    this.props.changeSortKey(["profit", "asc"]);
  };
    ...
    this.props.getCurrencyList(this, -1).then((flag: any) => {
      ...
      if (flag && this.props.path.indexOf(getRoleById(roleId) as string) !== -1) {
        this.props.getPlatformList(this).then((flag: any) => {
          ...
          this.props.getCountrysList(this);
  ...
  onSaveExcelButton = () => {
    ...
    let params: any = {
      ...
      countryIds: this.props.selectedCountrys.length === 0 ? '' : convertSelectedCountryValuesToString(this.props.selectedCountrys, this.props.countrysList),
    ...
    let filename: string;
    fetch(concatParamsToURL(api.GET_COUNTRY_REPORT_FILE, params), request)
      .then((response: any) => {
    ...
    const columns: any = [
      {
        ...
        render(value: any, row: any, i: number) {
          return {
            children: that.getLevelKey() === 1 ? <img className={"flag flag-" + `${value}`.toLowerCase()}/> : value,
            props: {
              style: {
                fontWeight: '700',
              }
      ...
      },
      {
        ...
        render: (value: any, row: any, i: number) => {
          ...
          return {
            // children: value, //this.renderNameCell(value, row.id),
            children: this.getLevelKey() < 2 ?
            <a onClick={() => this.goToLevel(level, value, row.id)}>{value}</a> : value,
            props: {
              style: {
                fontWeight: '700',
              }
  ...
  renderNameCell = (value: string, id: string|number) => {
    let title = (
    <div>
     {/*  <span className={"tooltip-link-to-report"} onClick={()=>{this.openReport("platform", value, id)}}>Platform</span>
      <br/> */}
      <span className={"tooltip-link-to-report"} onClick={()=>{this.openReport("currency", value, id)}}>Currency</span>
      <br/>
      <span className={"tooltip-link-to-report"} onClick={()=>{this.openReport("game", value, id)}}>Game</span>
      <br/>
      <span className={"tooltip-link-to-report"} onClick={()=>{this.openReport("device", value, id)}}>Device</span>
    </div>);
    return (
  ...
  openReport = (type: string, value: string, id: string|number) => {
    console.log(JSON.stringify({type, value, id}));
  ...
  renderFilter() {
      ...
      },
    {
        ...
        props: {
          allowClear: true,
          value: this.props.selectedCountrys,
          treeData: formatCountryData(this.props.countrysList),
          onChange: this.props.selectCountrys,
          showSearch: true,
          treeCheckable: true,
          showCheckedStrategy: TreeSelect.SHOW_PARENT,
          searchPlaceholder: (localization.PLEASE_SELECT as any)[lang]
    ...
    if(this.props.metadata.countryName) {
      tProps.pop();
    }
    //checkbox
    /* let checkbox = [
...
export const CountryReport = withRouter(CountryReportVO);
