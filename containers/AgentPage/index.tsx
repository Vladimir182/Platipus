...
class AgentPageVO extends React.Component<LoginPage.Props, LoginPage.State> {
  ...
  componentDidMount() {
    ...
    if (this.props.currentTab === roles.AGENT.route) {
      this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children["OFFICE_REPORT_" + VERSION]}?page=1&size=10`);
    } else {
      if (roleId != roles.AGENT.id) {
  ...
  openHelp = () => {
    let help_url = `${roles.AGENT.route}${roles.AGENT.children.HELP}`;
  ...
  changeVerticalNavBarTab(): string[] {
    switch (this.props.currentTab) {
      case `${roles.AGENT.route}${roles.AGENT.children["CURRENCIES_REPORT_" + VERSION]}`:
        return ['v-2'];
      case `${roles.AGENT.route}${roles.AGENT.children["GAMES_REPORT_" + VERSION]}`:
        return ['v-3'];
      case `${roles.AGENT.route}${roles.AGENT.children["OFFICE_REPORT_" + VERSION]}`:
        return ['v-1'];
      case `${roles.AGENT.route}${roles.AGENT.children.USERS_BACKOFFICE}`:
        return ['v-4'];
      case `${roles.AGENT.route}${roles.AGENT.children.USERS_CASINO}`:
        return ['v-5'];
      case `${roles.AGENT.route}${roles.AGENT.children.BETS_HISTORY}`:
        return ['v-6'];
      case `${roles.AGENT.route}${roles.AGENT.children.TRANSFERS_HISTORY}`:
        return ['v-61'];
      case `${roles.AGENT.route}${roles.AGENT.children.CURRENCY_SETTINGS}`:
        return ['v-7'];
      case `${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}`:
        return ['v-13'];
      case `${roles.AGENT.route}${roles.AGENT.children.DAY_REPORT}`:
        return ['v-14'];
      default: {
        return [];
      }
  ...
  pushToHistoryCurrencyReport() {
    ...
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children["CURRENCIES_REPORT_" + VERSION]}${search}`)
  }

  pushToHistoryGameReport() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children["GAMES_REPORT_" + VERSION]}${search}`)
  }

  pushToHistoryOfficeReport() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children["OFFICE_REPORT_" + VERSION]}${search}`)
  }

  pushToHistoryBackOfficeUsers() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.USERS_BACKOFFICE}${search}`)
  }

  pushToHistoryDayReport() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.DAY_REPORT}${search}`)
  }

  pushToHistoryCasinoUsers() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.USERS_CASINO}${search}`)
  }

  pushToHistoryBetsHistory() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.BETS_HISTORY}${search}`)
  }

  pushToHistoryTransfersHistory() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.TRANSFERS_HISTORY}${search}`)
  }

  pushToHistoryCurrencySetting() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.CURRENCY_SETTINGS}${search}`)
  }

  pushToHistoryStructure() {
    let search = this.resetPagination();
    this.props.history.push(`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}${search}`)
  }
      ...
      <div style={{width: '100%'}}>
        ...
        <Row>
          <NavBarHorizontal
            ...
            openAccount={openAccount(this, roles, 'AGENT')}
        ...
        <Row>
          ...
          <Col id={'affix-scrollable-content'} xs={24} sm={24} md={isMobileAndTablet ? 24 : 21} lg={isMobileAndTablet ? 24 : 21} xl={21}
            ...
            <Switch>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.HELP}`} component={Help}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.ACCOUNT}`} component={Account}/>

              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.CURRENCIES_REPORT_V3}`} component={CurrencyReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.CURRENCIES_REPORT_V3}/:currencyId`} component={CurrencyReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.CURRENCIES_REPORT_V3}/:currencyId/:officeId`} component={CurrencyReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.CURRENCIES_REPORT_V3}/:currencyId/:officeId/:roomId`} component={CurrencyReport}/>

              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.GAMES_REPORT_V3}`} component={GamesReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.GAMES_REPORT_V3}/:gameId`} component={GamesReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.GAMES_REPORT_V3}/:gameId/:platform`} component={GamesReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.GAMES_REPORT_V3}/:gameId/:platform/:officeId`} component={GamesReport}/>

              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.OFFICE_REPORT_V3}`} component={OfficeReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.OFFICE_REPORT_V3}/:platform`} component={OfficeReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.OFFICE_REPORT_V3}/:platform/:officeId`} component={OfficeReport}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.OFFICE_REPORT_V3}/:platform/:officeId/:roomId`} component={OfficeReport}/>

              <Route path={`${roles.AGENT.route}${roles.AGENT.children.DAY_REPORT}`} component={DayReport}/>

              <Route path={`${roles.AGENT.route}${roles.AGENT.children.USERS_BACKOFFICE}`} component={BackOfficeUsers} key={window.location.pathname}/>
              <Route path={`${roles.AGENT.route}${roles.AGENT.children.USERS_CASINO}`} component={BackOfficeUsers} key={window.location.pathname}/>
              <Route path={`${roles.AGENT.route}${roles.AGENT.children.BETS_HISTORY}`} component={BetsHistoryList} key={window.location.pathname}/>
              <Route path={`${roles.AGENT.route}${roles.AGENT.children.TRANSFERS_HISTORY}`} component={TransfersHistoryList} key={window.location.pathname}/>

              <Route path={`${roles.AGENT.route}${roles.AGENT.children.CURRENCY_SETTINGS}`} component={CurrencySettings}/>

              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.HISTORY_LIST}/:id`} component={BetsHistoryList} key={window.location.pathname}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.HISTORY_LIST}`} component={BetsHistoryList} key={window.location.pathname}/>
              <Route exact
                     path={`${roles.AGENT.route}${roles.AGENT.children.HISTORY_LIST}/:id${roles.AGENT.children.HISTORY_DETAILS}/:recordId`}
                     component={HistoryDetails}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}`} component={Structure}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}/:platform`} component={Structure}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}/:platform/:distributorId`} component={Structure}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}/:platform/:distributorId/:officeId`} component={Structure}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}/:platform/:distributorId/:officeId/:agentId`} component={Structure}/>
              <Route exact path={`${roles.AGENT.route}${roles.AGENT.children.STRUCTURE}/:platform/:distributorId/:officeId/:agentId/:roomId`} component={Structure}/>
            </Switch>
          </Col>
        </Row>
      </div>);
...
export const AgentPage = withRouter(AgentPageVO);
